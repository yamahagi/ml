...
| EMul (e1,e2) ->
  ...
  |EPair (e1,e2) ->
	let (v1,const1) = infer_expr env e1 in
	let (v2,const2) = infer_expr env e1 in
...
let rec infer_cmd tyenv c =
  ...
  let x1 = ty_subst (unify const) x in
  (x1,tyenv)	
  | CDecl (e1,e2) -> 
  let (t,const) = (infer_expr tyenv e2) in
  let tx = (ty_subst (unify const) t) in
   (tx,(e1,tx)::tyenv)
  | CRecDecl (f,x,e1) ->
  let newvarx = TyVar (new_tyvar()) in
        ...
        let (v1,const1)= infer_expr newenv e1 in
	let fvar = ty_subst (unify((v1,newvare1)::const1)) (TyFun(newvarx,newvare1)) in
	(fvar,(f,fvar)::tyenv)
