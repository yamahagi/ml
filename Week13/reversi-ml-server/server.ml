...
let options = [
  ...
  ("-t", Arg.Set_int opt_players_time, " time assigned to each player (in second)");
  ("--timeout", Arg.Set_int opt_timeout, " time out (in millisecond)");
  ("-T", Arg.Set_int opt_byoyomi, " byoyomi time");
  ("-n", Arg.Set_int opt_players_number, " number of players");
  ("-r", Arg.Set_int opt_rounds, " number of rounds (multiple of 2)");
  ("-c", Arg.Set_int opt_concurrency, " number of maximum concurrent matches");
]
let hidden = [
  ("--port", Arg.Set_int opt_port, " port number");
  ("--playerstime", Arg.Set_int opt_players_time, " time assigned to each player (in second)");
  ("--byoyomi", Arg.Set_int opt_byoyomi, " byoyomi time");
  ("--players", Arg.Set_int opt_players_number, " number of players");
  ("--rounds", Arg.Set_int opt_rounds, " number of rounds (multiple of 2)");
  ("--concurrency", Arg.Set_int opt_concurrency, " number of maximum concurrent matches");
  ...
  in
    ...
    with
      | PlayInvalidCommand cl ->
          let black_result =
            if name_of_client cl = name_of_client bc then
              "LOSE"
            else
              "WIN" in
          let white_result =
            if name_of_client cl = name_of_client wc then
              "LOSE"
            else
              "WIN" in
